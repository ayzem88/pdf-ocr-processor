...
# ===================== تدوير الصفحات (مساعدات) =====================

def _normalize_degrees(degrees: int) -> int:
    """يطبع الزاوية إلى إحدى القيم {0,90,180,270}."""
    d = degrees % 360
    if d < 0:
        d += 360
    if d in (0, 90, 180, 270):
        return d
    # تقريب لأقرب مضاعف 90 (حذرًا):
    return ((round(d / 90)) * 90) % 360

def _rotate_page_inplace(page, degrees: int):
    """يدوّر الصفحة بمقدار 0/90/180/270 مع دعم واجهات PyPDF2 المتعددة.
    يُعدّل الصفحة في المكان ويعيدها.
    """
    deg = _normalize_degrees(degrees)
    if deg == 0:
        return page
    # أولاً: واجهة PyPDF2 3.0.0+ الموحّدة
    try:
        page.rotate(deg)
        return page
    except Exception:
        pass
    # محاولة واجهة PyPDF2 الحديثة (snake_case)
    try:
        if deg == 90:
        ...
        pass
    # محاولة الواجهة القديمة (camelCase)
    try:
        if deg == 90:
        ...
        pass
    # ملاذ أخير: تعديل خاصية /Rotate مباشرة
    try:
        current = page.get("/Rotate") or 0
        page["/Rotate"] = (int(current) + deg) % 360
    except Exception:
        # تجاهل بصمت إذا فشل كل شيء
        pass
    return page
...
class PDFService:
    ...
    def rotate_pages(self, pages_spec: str, degrees: int) -> Path:
        """يدوّر صفحات محددة من أول ملف PDF في المجلد بزوايا 0/90/180/270.
        الصفحات غير المحددة تبقى كما هي.
        """
        doc = self._open_first()
        if doc.total_pages == 0:
            raise ValueError("الملف لا يحتوي على صفحات.")
        indices = parse_pages(pages_spec, doc.total_pages)
        deg = _normalize_degrees(degrees)
        if deg not in (0, 90, 180, 270):
            raise ValueError("الزاوية يجب أن تكون من مضاعفات 90.")

        assert doc.reader is not None
        writer = PdfWriter()
        for i in range(doc.total_pages):
            page = doc.reader.pages[i]
            if i in indices and deg != 0:
                page = _rotate_page_inplace(page, deg)
            writer.add_page(page)

        deg_label = {0:"0", 90:"90", 180:"180", 270:"270"}[deg]
        out_name = f"{doc.base_name}-مدوّر-{deg_label}-صفحات-{pages_spec.replace(' ','')}.pdf"
        out = self.directory / out_name
        return PDFExporter(self.directory).export_writer(writer, out)

    def reorder_pages(self, ascending: bool = True) -> Path:
        """يعيد ترتيب كل صفحات أول ملف PDF: تصاعدي (أ-ي) أو تنازلي (ي-أ)."""
        doc = self._open_first()
        if doc.total_pages == 0:
            raise ValueError("الملف لا يحتوي على صفحات.")
        ...
        order_iter = range(doc.total_pages) if ascending else range(doc.total_pages - 1, -1, -1)
        for i in order_iter:
            writer.add_page(doc.reader.pages[i])

        mode_label = "تصاعدي" if ascending else "تنازلي"
        out = self.directory / f"{doc.base_name}-معاد_الترتيب-{mode_label}.pdf"
        return PDFExporter(self.directory).export_writer(writer, out)
...
def run_rotate_pages(directory: Path):
    """يدوّر صفحات محددة بزوايا 0/90/180/270 من أول ملف PDF."""
    try:
        svc = PDFService(directory)
        spec = input("أدخل الصفحات للتدوير بصيغة 1,3,5-8: ").strip()
        deg_raw = input("أدخل زاوية التدوير (0، 90، 180، 270): ").strip()
        try:
            deg_val = int(normalize_digits(deg_raw))
        except ValueError:
            print("قيمة الزاوية غير صحيحة.")
            return
        out = svc.rotate_pages(spec, deg_val)
        print(f"تم التدوير وحفظ الناتج: {out.name}")
    except Exception as e:
        print(f"فشل تدوير الصفحات: {e}")

def run_reorder_pages(directory: Path):
    """يعيد ترتيب كل صفحات أول ملف PDF: أ-ي أو ي-أ."""
    try:
        svc = PDFService(directory)
        mode = input("اختر الترتيب: 1) أ-ي (تصاعدي)  2) ي-أ (تنازلي): ").strip()
        if mode == "1":
            out = svc.reorder_pages(ascending=True)
        elif mode == "2":
            out = svc.reorder_pages(ascending=False)
        else:
            print("خيار غير صحيح.")
            return
        print(f"تم إعادة الترتيب وحُفظ الناتج: {out.name}")
    except Exception as e:
        print(f"فشل إعادة الترتيب: {e}")
...
def main():
    ...
    MENU = (
        ...
        "11) تدوير صفحات من أول ملف مصور\n"
        "12) إعادة ترتيب صفحات أول ملف مصور (أ-ي / ي-أ)\n"
        "13) إنهاء البرنامج\n"
        "إدخال: "
    )
    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                run_rotate_pages(base_dir)
            elif choice == "12":
                run_reorder_pages(base_dir)
            elif choice == "13":
                print("تم الإنهاء.")
                break
            else:
